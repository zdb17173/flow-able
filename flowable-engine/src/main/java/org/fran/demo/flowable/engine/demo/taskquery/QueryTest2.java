package org.fran.demo.flowable.engine.demo.taskquery;
    ...
    public static void main(String[] args) {
        ...
        for( FlowElement element : elements) {
            if( element instanceof UserTask) {
                ...
                System.out.println(tid + " " + tname + " " + tgroup);
            }
        }
        ...
        param.put("dsada", "dsadsa");//processVariable字段
        ...
        edit(tasks, curUser);
        submit(tasks, false, "kkkk", null);


        //第二个节点
        curUser = "n21";//n21
        tasks = taskService.createTaskQuery()
                .processVariableValueEquals("query", "kkkk")
                .taskCandidateGroup(curUser)
                .includeProcessVariables()//包含variables
                .list();
        for(Task t : tasks){
            Map<String, Object> var = t.getProcessVariables();
            System.out.println(var);
        }

        submit(tasks, false, "k1", null);

        //第三个节点
        curUser = "n31";
        tasks = taskService.createTaskQuery()
                .processVariableValueEquals("query", "k1")
                .taskCandidateGroup(curUser).list();
        edit(tasks, curUser);
//        back(tasks, 2 ,false);
        submit(tasks);
    ...
    public static void submit(List<Task> tasks, boolean end){
        submit(tasks, end, null, null);
    }

    public static void submit(List<Task> tasks, boolean end, String key, String... nextNodeCandidate){
        ...
        if(tasks.size()>0){
            ...
            if(instance!= null) {
                List<Task> nextTasks = taskService.createTaskQuery()
                        ...
                        .list();
                if(nextTasks.size()>0){
                    Task ttt = nextTasks.get(0);
                    if(key!= null)
                        taskService.setVariable(ttt.getId(), "query", key);
                }
